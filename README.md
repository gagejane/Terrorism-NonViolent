...
## Data Description
For this project, I used OEF’s Terrorist and Insurgent Organizations’ Service Provision across time (TIOS 2.0) dataset for this project (for more information, see Heger, Jung, and Wagstaff, 2018). The TIOS 2.0 dataset provides indicators associated with whether and how more than 400 violent non-state actors provide public goods and services across more than four decades (1969–2013). Accoring to OEF researchers, an underlying assumption of these data is that service-providing groups will have, on average, more stories mentioning service provision compared with groups that provide fewer or no services (Heger & Jung, 2017). To the best of OEF’s knowledge, this dataset represents the widest coverage across both time and groups for this type of data.
...
Using the world dataframe, GeoPandas builds a template that can then apply to their own data (see Figure 4 for template).

![World](World.png)

**Figure 4**. World template using GeoPandas built-in "world" dataframe.

To work with this template, I merged my pandas dataframe with the world dataframe, matching on country name. My dataframe contained the count of the total number of times non-state actors provided services, aggregated across year, into the TIOS 2.0 dataframe. See Figure 5 for the resulting heatmap, which represents the prevalence of non-state actors' services over the past 45 years.
...
**Table 2**. A count of the number of years groups in the U.S.
...
**Figure 6**. A count of the number of articles for each U.S. group.

Given that the Ku Klux Klan is the most prevalent group in the U.S., I explored their provision rate by type, over time. As seen in Figure 7, social provisions were by far the most prevalent for each group.
...
To take a closer look at the prevalence of non-social provisions for the Ku Klux Klan, I re-plotted Figure 7, this time omitting social provisions.
